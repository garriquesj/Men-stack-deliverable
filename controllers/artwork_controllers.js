...
const db = require('../models')//accesses 
...
router.get('/', (req, res) => {
    db.Artwork.find({}, (error,foundArt) => {
    if (error) {
        console.log(error);
} 
    const context = {artwork:foundArt}
    res.render("art_index.ejs", {artwork:foundArt});
})
});
...
res.render('art_show.ejs', {artwork: singleArt });
})
});
...
db.Artwork.findByIdAndUpdate(req.params.artistid, req.body,(error, updatedArtist) => {
